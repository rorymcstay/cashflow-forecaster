...
day_dv = DataValidation(
    ...
    error="Enter a day of month between 1 and 31, or leave blank.",
)
...
# --- INCOME ---
section_row(row, "INCOME")
row += 1
income_start = row
item_row(row, "Income", "Household member 1 (take-home)", 0, note="Fill in")
row += 1
item_row(row, "Income", "Household member 2 (take-home)", 0, note="Fill in")
row += 1
item_row(row, "Income", "Other income", 0, note="e.g. side income, bonuses")
row += 1
# a few blank editable rows for additional income sources
for _ in range(3):
    item_row(row, "Income", "", 0)
    row += 1
income_end = row - 1
income_total_row = row
subtotal_row(row, "Total Income", income_start, income_end)
row += 2

# --- HOUSING ---
section_row(row, "HOUSING")
row += 1
housing_start = row
item_row(
    row,
    ...
    2800,
    day=17,
    note="Joint account — TLP RE Client Acc (letting agent), confirmed 12/12 months",
)
row += 1
item_row(
    row,
    ...
    "Other housing costs",
    0,
    note="Council Tax is paid as an annual lump sum — see Upcoming Expenses (~£1,020, due ~Apr)",
)
row += 1
housing_end = row - 1
housing_total_row = row
subtotal_row(row, "Total Housing", housing_start, housing_end)
row += 2

# --- UTILITIES & INSURANCE ---
section_row(row, "UTILITIES & INSURANCE")
row += 1
util_start = row
item_row(
    row,
    ...
    68.76,
    day=1,
    note="E.ON Next DD, Joint account — actual avg £68.76 (range £62-76), was budgeted £64",
)
row += 1
item_row(
    row, "Utilities", "WiFi", 20.56, day=5, note="Community Fibre DD, Joint account — confirmed, very stable"
)
row += 1
item_row(
    row,
    ...
    "Home insurance",
    23,
    note="Not found in reviewed statements — verify still active / correct amount",
)
row += 1
item_row(
    row,
    ...
    23.02,
    day=9,
    note="Urban Jungle DD, Joint account — renewed from £15.02 to £23.02/mo in Apr 2026",
)
row += 1
util_end = row - 1
util_total_row = row
subtotal_row(row, "Total Utilities & Insurance", util_start, util_end)
row += 2

# --- TRANSPORT ---
section_row(row, "TRANSPORT")
row += 1
transport_start = row
item_row(
    row,
    ...
    31.50,
    day=1,
    note='Personal HSBC DD — was bundled into "Car expenses 142"',
)
row += 1
item_row(
    row,
    ...
    32.81,
    day=1,
    note='Personal HSBC DD — second car, was bundled into "Car expenses 142"',
)
row += 1
item_row(
    row,
...
item_row(
    ...
    "Fuel",
    102.54,
    note='Variable card spend, Joint account — avg per active month, not a fixed DD (was "Car expenses 100")',
)
row += 1
transport_end = row - 1
transport_total_row = row
subtotal_row(row, "Total Transport", transport_start, transport_end)
row += 2

# --- SUBSCRIPTIONS ---
section_row(row, "SUBSCRIPTIONS")
row += 1
sub_start = row
item_row(row, "Subscriptions", "Spotify", 17.66, day=1, note="Personal Monzo — confirmed, day varies 1-11")
row += 1
item_row(row, "Subscriptions", "Netflix", 5.99, day=5, note="Joint account — confirmed exact")
row += 1
item_row(row, "Subscriptions", "Amazon Prime", 8.99, day=16, note="Amex — confirmed exact")
row += 1
item_row(
    row,
    "Subscriptions",
    "Apple",
    3,
    note="Not confirmed monthly — only a one-off £53.99 Amex charge seen; verify what this is",
)
row += 1
item_row(row, "Subscriptions", "Google", 2, note="Not found in reviewed statements — verify still active")
row += 1
item_row(
    row,
    "Subscriptions",
    ...
    36.00,
    day=1,
    note="TV Licensing DD, Joint account — actual is £36, was budgeted £15; only seen Jun & Jul (may be a 10-month DD scheme)",
)
row += 1
item_row(
    row,
    ...
    "AWS",
    23.53,
    note="Usage-based, Personal Monzo — avg £23.53 (range ~£14-33), varies month to month, was budgeted £5",
)
row += 1
item_row(
    row,
    ...
    18.00,
    day=28,
    note="New — Amex, confirmed 3/3 months, not in original list",
)
row += 1
item_row(
    row,
    ...
    10.00,
    day=8,
    note="New — Personal HSBC, variable £4.80-16, present every month, not in original list",
)
row += 1
sub_end = row - 1
sub_total_row = row
subtotal_row(row, "Total Subscriptions", sub_start, sub_end)
row += 2

# --- PERSONAL & OTHER ---
section_row(row, "PERSONAL & OTHER")
row += 1
personal_start = row
item_row(
    row,
    ...
    "Nathan",
    150,
    note="Not found in reviewed statements — verify (may be paid via cash/another account)",
)
row += 1
item_row(
    row,
...
item_row(
    ...
    "The Riders Hub",
    15.00,
    note="Low confidence — appears sporadically on personal HSBC & Monzo, possibly a lesson/hobby subscription; verify",
)
row += 1
personal_end = row - 1
personal_total_row = row
subtotal_row(row, "Total Personal & Other", personal_start, personal_end)
row += 2

# --- SAVINGS & INVESTMENTS CONTRIBUTION ---
section_row(row, "SAVINGS & INVESTMENTS")
row += 1
savings_start = row
item_row(row, "Savings", "Monthly savings contribution", 0, note="Fill in")
row += 1
item_row(row, "Investments", "Monthly investment contribution", 0, note="Fill in")
row += 1
savings_end = row - 1
savings_total_row = row
subtotal_row(row, "Total Savings & Investments", savings_start, savings_end)
row += 2

# --- SUMMARY ---
section_row(row, "SUMMARY")
row += 1
...
for col in (3, 4, 5, 6):
    ...
    c = ws2.cell(
        row=total_row, column=col, value=f"=SUM({colletter}{data_start}:{colletter}{data_end_extended})"
    )
    c.number_format = CURRENCY
    c.font = TOTAL_FONT
    c.fill = TOTAL_FILL
...
ws_ue["A2"] = (
    "One-off items that crop up as and when they come up (e.g. car MOT, a gift, a repair). "
...
ws3["A2"] = (
    "Set Forecast Month + Opening Balance below. Realised Net overrides Forecast Net for a day when filled in. "
...
for n in range(1, 32):
    ...
    ws3.cell(row=r, column=1, value=f'=IF({n}<=DAY(EOMONTH($B$4,0)),DATE(YEAR($B$4),MONTH($B$4),{n}),"")')
...
print(
    f"BUDGET_LAST_ROW={BUDGET_LAST_ROW}  UE_FIRST_ROW={UE_FIRST_ROW}  UE_LAST_ROW={UE_LAST_ROW}  "
